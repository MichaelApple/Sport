...
<?php 
/**
*Template Name: Find_All_Agencies_Proposals
**/
 ?>
 <?php 
include ('db.php');
...
//                   <p>'.$row_search['agency_state'].'</p>'.$row_search['agency_notes'].'&nbsp;'.$row_search['agency_adress1'].' '.$row_search['agency_city1'].'
//                 </div>
//               </div>
//             </div>';
//     }
...
<form id="left-search" method="POST" action="http://commercia/find_agencies/" enctype="multipart/form-data">  
      <p>Filter by:</p>
      <p>Location</p>
      <p>State*
        ...
        <input class="form-control" type="text" name="state" list="state_list"></p>
...
$result2 = mysql_query("SELECT * FROM agency;");
...
if ($page>=1) {
  ...
  echo '<li><a href="find_agencies/?page=1">To the start</a> &nbsp; '; //Це на саму першу сторінку
   
  echo '<li><a href="find_agencies/?page=' . $page . '">Previous </a>  ';
}
...
if ($j>$page && ($page+2)<$j) {
        ...
        echo '<li><a href="find_agencies/?page=' . ($page+2) . '"> Next</a> &nbsp; ';
         echo '<a href="find_agencies/?page=' . ($j-1) . '">To the end</a> &nbsp; ';
}

if(!isset($_POST['page_search'])) {
  // Запит в базу данних із змінними $quantity и $list
  $result = mysql_query("SELECT * FROM agency ORDER BY id DESC
              ...
              <div class="search-container">
                <div class="row">
                  <div class="col-sm-4 logo">
                    ...
                    <img src="http://commercia/wp-content/themes/Commerce/uploads/agency/'.$row['agency_logo'].'" width="200px" height="100%">
                  </div>
                  <div class="col-sm-8" style="text-align: left;">
                    <p>'.$row['agency_state'].'</p>'.$row['agency_notes'].'&nbsp;'.$row['agency_adress1'].' '.$row['Agency_city1'].'
                  </div>
                </div>
              </div>';
  }
  ...
  if(isset($_POST['page_search'])) {
            ...
            $query = mysql_query("SELECT * FROM agency  WHERE agency_state LIKE '%$search_state%'  ORDER BY id DESC" ) or die('There is no matches!');  
            $count = mysql_num_rows($query);
            // Виводимо всі записи даної сторінки
            for ($l = 0; $l<$count; $l++) {
               ...
               echo '<div class="search-container">
                        ...
                        <div class="col-sm-8" style="text-align: left;">
                          <p>'.$row_search['agency_state'].'</p>'.$row_search['agency_notes'].'&nbsp;'.$row_search['agency_adress'].' '.$row_search['agency_city'].'
                        <p>'.$count.'</p>
                        </div>
                      </div>
                    </div>';
